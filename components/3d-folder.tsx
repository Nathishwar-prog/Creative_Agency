...
const ImageLightbox: React.FC<ImageLightboxProps> = React.memo(({
  projects,
  currentIndex,
  isOpen,
  onClose,
        ...
        <div className={cn("relative overflow-hidden rounded-[inherit] bg-card border border-white/10 shadow-[0_35px_60px_-15px_rgba(0,0,0,0.5)]")}>
          ...
          >
            <div className="flex items-center justify-between gap-6">
              ...
              <Link href={currentProject?.link || "#"} target="_blank" className={cn("flex items-center gap-2 px-6 py-3 text-sm font-bold uppercase tracking-widest text-primary-foreground bg-primary hover:brightness-110 rounded-xl shadow-lg shadow-primary/20 transition-all duration-300 hover:scale-105 active:scale-95")}>
...
const AnimatedFolder: React.FC<AnimatedFolderProps> = React.memo(({ title, projects, className, gradient }) => {
  ...
  return (
    <>
      <div
        ...
        onBlur={(e) => {
          // Only close if focus moves outside the folder
        ...
        <div className="relative flex items-center justify-center mb-4" style={{ height: "160px", width: "200px" }}>
          ...
          <div className="absolute" style={{ top: "50%", left: "50%", transform: "translate(-50%, -50%)", zIndex: 20 }}>
            {previewProjects.map((project, index) => (
              <MemoizedProjectCard key={project.id} ref={(el) => { cardRefs.current[index] = el; }} image={project.image} title={project.title} delay={index * 50} isVisible={isHovered} index={index} totalCount={previewProjects.length} onClick={() => handleProjectClick(project, index)} isSelected={selectedProject?.id === project.id && isDetailsOpen} />
            ))}
          </div>
...
const portfolioData = worksData as { title: string; gradient: string; projects: Project[] }[];

export default function Folder() {
  // Manual theme logic removed to prevent conflict with global provider
  ...
  return (
    <main className="min-h-screen bg-background text-foreground transition-colors duration-500 selection:bg-accent/30 selection:text-accent-foreground ">
      ...
      <div className="max-w-7xl mx-auto mt-20 pt-20 px-6 text-center">
        <h1 className="text-5xl md:text-7xl font-black uppercase tracking-tighter mb-4 animate-in fade-in slide-in-from-bottom-4 duration-700">
          Our <span className="text-primary italic text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-600">Works</span>
        </h1>
